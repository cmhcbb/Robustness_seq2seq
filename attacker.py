...
class Translator(object):
    ...
    def get_loss(self, batch, data, inpt_src):
        #batch_data, _ = self.translate_batch(batch, data, fast=self.fast, inpt_src=inpt_src, attack=True)
    ...
    def translate_batch(self, batch, data, fast=False, inpt_src=None, attack=False):
        """
        ...
        with torch.no_grad():
            if fast:
                ...
                #print("It's normal")
                return self._translate_batch(batch, data, inpt_src=inpt_src, attack=attack)

    def _run_encoder(self, batch, data_type, inpt_src=None, attack=False):
        if not attack:
    ...
    def _translate_batch(self, batch, data, inpt_src=None, attack=False):
        ...
        # (1) Run the encoder on the src.
        src, enc_states, memory_bank, src_lengths = self._run_encoder(batch, data_type, inpt_src=inpt_src, attack=attack)
